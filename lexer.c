...
void print_unknown_pattern(int token_start, int current_ind, int line, char* buffer) {
    ...
    int len;
    if(end_idx >= token_start)
        len = end_idx - token_start + 1;
    else
        len = TOTAL_B_SIZE - token_start + end_idx + 1;
...
Token create_token(int token_start, int token_end, TokenType type, int current_line) {
    ...
    if(token_end >= token_start)
        ...
        len = TOTAL_B_SIZE - token_start + token_end + 1;
...
Token getNextToken() {
    ...
    while(!is_eof && !token_ready) {
        ...
        switch(current_state) {
                ...
                if(c == ' ' || c == '\t' || c == '\r' || c == '\n') {
                    ...
                    switch(c) {
                        ...
                        case '%':
                            ...
                            break;
                        case '[':
                            generated_token = create_token(token_start, ind, TK_SQL, line_number);
                            token_ready = true;
                            break;
                        case ']':
                            generated_token = create_token(token_start, ind, TK_SQR, line_number);
                            token_ready = true;
                            break;
                        case ',':
                            generated_token = create_token(token_start, ind, TK_COMMA, line_number);
                            token_ready = true;
                            break;
                        case ';':
                            generated_token = create_token(token_start, ind, TK_SEM, line_number);
                            token_ready = true;
                            break;
                        case '.':
                            generated_token = create_token(token_start, ind, TK_DOT, line_number);
                            token_ready = true;
                            break;
                        case ':':
                            generated_token = create_token(token_start, ind, TK_COLON, line_number);
                            token_ready = true;
                            break;
                        case '/':
                            generated_token = create_token(token_start, ind, TK_DIV, line_number);
                            token_ready = true;
                            break;
                        case '+':
                            generated_token = create_token(token_start, ind, TK_PLUS, line_number);
                            token_ready = true;
                            break;
                        case '-':
                            generated_token = create_token(token_start, ind, TK_MINUS, line_number);
                            token_ready = true;
                            break;
                        case '*':
                            generated_token = create_token(token_start, ind, TK_MUL, line_number);
                            token_ready = true;
                            break;
                        case '~':
                            generated_token = create_token(token_start, ind, TK_NOT, line_number);
                            token_ready = true;
                            break;
                        case '(':
                            generated_token = create_token(token_start, ind, TK_OP, line_number);
                            token_ready = true;
                            break;
                        case ')':
                            generated_token = create_token(token_start, ind, TK_CL, line_number);
                            token_ready = true;
                            break;
                        case '@':
                            current_state = 35;
            ...
            case 45:
                if(c == '\n') {
                    generated_token = create_token(token_start, token_start, TK_COMMENT, line_number);
                    line_number++;
                    token_ready = true;
                } else
                    break;
    ...
    char* eof_str = (char*)malloc(sizeof(char));
    eof_str[0] = '\0';
    Token t = {.type = TK_EOF, .line_number = line_number, .token = eof_str};
    return t;
}
