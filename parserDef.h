...
static inline char* get_variable_name(Variable var) {
    switch(var) {
        case VAR_PROGRAM:
            return "PROGRAM";
        ...
        case VAR_ACTUAL_OR_REDEFINED:
            return "ACTUAL_OR_REDEFINED";
        ...
        case VAR_OPTION_SINGLE_CONSTRUCTED:
            return "OPTION_SINGLE_CONSTRUCTED";
        case VAR_ONE_EXPANSION:
            return "ONE_EXPANSION";
        case VAR_MORE_EXPANSIONS:
            return "MORE_EXPANSIONS";
        ...
        case VAR_FACTOR:
            return "FACTOR";
        case VAR_HIGH_PRECEDENCE_OPERATORS:
            return "HIGH_PRECEDENCE_OPERATORS";
        case VAR_LOW_PRECEDENCE_OPERATORS:
            return "LOW_PRECEDENCE_OPERATORS";
...
#define RULE(lhs, ...) {.root = lhs, .expansion = (Term[]){__VA_ARGS__}, .expansion_length = sizeof((Term[]){__VA_ARGS__}) / sizeof(Term)}
#define EPSILON_RULE(lhs) {.root = lhs, .expansion = NULL, .expansion_length = 0}
...
static const GrammarRule grammar[] = {
    ...
    RULE(VAR_OTHER_FUNCTIONS, V(VAR_FUNCTION), V(VAR_OTHER_FUNCTIONS)), EPSILON_RULE(VAR_OTHER_FUNCTIONS),
    ...
    RULE(VAR_OUTPUT_PAR, T(TK_OUTPUT), T(TK_PARAMETER), T(TK_LIST), T(TK_SQL), V(VAR_PARAMETER_LIST), T(TK_SQR)), EPSILON_RULE(VAR_OUTPUT_PAR),
    ...
    RULE(VAR_DATA_TYPE, V(VAR_PRIMITIVE_DATATYPE)), RULE(VAR_DATA_TYPE, V(VAR_CONSTRUCTED_DATATYPE)),

    // 9. <primitiveDatatype> ===> TK_INT | TK_REAL
    RULE(VAR_PRIMITIVE_DATATYPE, T(TK_INT)), RULE(VAR_PRIMITIVE_DATATYPE, T(TK_REAL)),

    // 10. <constructedDatatype> ===> TK_RECORD TK_RUID | TK_UNION TK_RUID | TK_RUID
    RULE(VAR_CONSTRUCTED_DATATYPE, T(TK_RECORD), T(TK_RUID)), RULE(VAR_CONSTRUCTED_DATATYPE, T(TK_UNION), T(TK_RUID)),
    RULE(VAR_CONSTRUCTED_DATATYPE, T(TK_RUID)),

    // 11. <remaining_list> ===> TK_COMMA <parameter_list> | epsilon
    RULE(VAR_REMAINING_LIST, T(TK_COMMA), V(VAR_PARAMETER_LIST)), EPSILON_RULE(VAR_REMAINING_LIST),
    ...
    RULE(VAR_TYPE_DEFINITIONS, V(VAR_ACTUAL_OR_REDEFINED), V(VAR_TYPE_DEFINITIONS)), EPSILON_RULE(VAR_TYPE_DEFINITIONS),

    // 13b. <actualOrRedefined> ===> <typeDefinition> | <definetypestmt>
    RULE(VAR_ACTUAL_OR_REDEFINED, V(VAR_TYPE_DEFINITION)), RULE(VAR_ACTUAL_OR_REDEFINED, V(VAR_DEFINETYPESTMT)),
    ...
    RULE(VAR_FIELD_TYPE, V(VAR_PRIMITIVE_DATATYPE)), RULE(VAR_FIELD_TYPE, V(VAR_CONSTRUCTED_DATATYPE)),

    // 18. <moreFields> ===> <fieldDefinition> <moreFields> | epsilon
    RULE(VAR_MORE_FIELDS, V(VAR_FIELD_DEFINITION), V(VAR_MORE_FIELDS)), EPSILON_RULE(VAR_MORE_FIELDS),

    // 19. <declarations> ===> <declaration> <declarations> | epsilon
    RULE(VAR_DECLARATIONS, V(VAR_DECLARATION), V(VAR_DECLARATIONS)), EPSILON_RULE(VAR_DECLARATIONS),
    ...
    RULE(VAR_GLOBAL_OR_NOT, T(TK_COLON), T(TK_GLOBAL)), EPSILON_RULE(VAR_GLOBAL_OR_NOT),

    // 22. <otherStmts> ===> <stmt> <otherStmts> | epsilon
    RULE(VAR_OTHER_STMTS, V(VAR_STMT), V(VAR_OTHER_STMTS)), EPSILON_RULE(VAR_OTHER_STMTS),

    // 23. <stmt> ===> <assignmentStmt> | <iterativeStmt> | <conditionalStmt> | <ioStmt> | <funCallStmt>
    RULE(VAR_STMT, V(VAR_ASSIGNMENT_STMT)), RULE(VAR_STMT, V(VAR_ITERATIVE_STMT)), RULE(VAR_STMT, V(VAR_CONDITIONAL_STMT)),
    RULE(VAR_STMT, V(VAR_IO_STMT)), RULE(VAR_STMT, V(VAR_FUN_CALL_STMT)),
    ...
    EPSILON_RULE(VAR_OPTION_SINGLE_CONSTRUCTED), RULE(VAR_OPTION_SINGLE_CONSTRUCTED, V(VAR_ONE_EXPANSION), V(VAR_MORE_EXPANSIONS)),
    ...
    RULE(VAR_MORE_EXPANSIONS, V(VAR_ONE_EXPANSION), V(VAR_MORE_EXPANSIONS)), EPSILON_RULE(VAR_MORE_EXPANSIONS),
    ...
    RULE(VAR_OUTPUT_PARAMETERS, T(TK_SQL), V(VAR_ID_LIST), T(TK_SQR), T(TK_ASSIGNOP)), EPSILON_RULE(VAR_OUTPUT_PARAMETERS),
    ...
    RULE(VAR_CONDITIONAL_STMT, T(TK_IF), T(TK_OP), V(VAR_BOOLEAN_EXPRESSION), T(TK_CL), T(TK_THEN), V(VAR_STMT), V(VAR_OTHER_STMTS),
         V(VAR_ELSE_PART)),

    // 31. <elsePart> ===> TK_ELSE <stmt> <otherStmts> TK_ENDIF | TK_ENDIF
    RULE(VAR_ELSE_PART, T(TK_ELSE), V(VAR_STMT), V(VAR_OTHER_STMTS), T(TK_ENDIF)), RULE(VAR_ELSE_PART, T(TK_ENDIF)),
    ...
    RULE(VAR_EXP_PRIME, V(VAR_LOW_PRECEDENCE_OPERATORS), V(VAR_TERM), V(VAR_EXP_PRIME)), EPSILON_RULE(VAR_EXP_PRIME),
    ...
    RULE(VAR_TERM_PRIME, V(VAR_HIGH_PRECEDENCE_OPERATORS), V(VAR_FACTOR), V(VAR_TERM_PRIME)), EPSILON_RULE(VAR_TERM_PRIME),

    // 38. <factor> ===> TK_OP <arithmeticExpression> TK_CL | <var>
    RULE(VAR_FACTOR, T(TK_OP), V(VAR_ARITHMETIC_EXPRESSION), T(TK_CL)), RULE(VAR_FACTOR, V(VAR_VAR)),

    // 39. <highPrecedenceOperators> ===> TK_MUL | TK_DIV
    RULE(VAR_HIGH_PRECEDENCE_OPERATORS, T(TK_MUL)), RULE(VAR_HIGH_PRECEDENCE_OPERATORS, T(TK_DIV)),

    // 40. <lowPrecedenceOperators> ===> TK_PLUS | TK_MINUS
    RULE(VAR_LOW_PRECEDENCE_OPERATORS, T(TK_PLUS)), RULE(VAR_LOW_PRECEDENCE_OPERATORS, T(TK_MINUS)),
    ...
    RULE(VAR_VAR, V(VAR_SINGLE_OR_REC_ID)), RULE(VAR_VAR, T(TK_NUM)), RULE(VAR_VAR, T(TK_RNUM)),

    // 45. <logicalOp> ===> TK_AND | TK_OR
    RULE(VAR_LOGICAL_OP, T(TK_AND)), RULE(VAR_LOGICAL_OP, T(TK_OR)),

    // 46. <relationalOp> ===> TK_LT | TK_LE | TK_EQ | TK_GT | TK_GE | TK_NE
    RULE(VAR_RELATIONAL_OP, T(TK_LT)), RULE(VAR_RELATIONAL_OP, T(TK_LE)), RULE(VAR_RELATIONAL_OP, T(TK_EQ)), RULE(VAR_RELATIONAL_OP, T(TK_GT)),
    RULE(VAR_RELATIONAL_OP, T(TK_GE)), RULE(VAR_RELATIONAL_OP, T(TK_NE)),
    ...
    RULE(VAR_OPTIONAL_RETURN, T(TK_SQL), V(VAR_ID_LIST), T(TK_SQR)), EPSILON_RULE(VAR_OPTIONAL_RETURN),

    // 49. <idList> ===> TK_ID <more_ids>
    RULE(VAR_ID_LIST, T(TK_ID), V(VAR_MORE_IDS)),

    // 50. <more_ids> ===> TK_COMMA <idList> | epsilon
    RULE(VAR_MORE_IDS, T(TK_COMMA), V(VAR_ID_LIST)), EPSILON_RULE(VAR_MORE_IDS),
    ...
    RULE(VAR_A, T(TK_RECORD)), RULE(VAR_A, T(TK_UNION))};
